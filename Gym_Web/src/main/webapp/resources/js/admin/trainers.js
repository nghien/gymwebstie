function POST() {
	...
	data.id = 0;
	$.ajax({
		url : "/SpringRestHibernateExample/addTrainers",
		method : "POST",
...
function PUT() {
	var data = $("#form").serializeFormJSON();
	$.ajax({
		url : "/SpringRestHibernateExample/updateTrainers",
		method : "PUT",
		contentType : "application/json",
		data : JSON.stringify(data),
		success : function() {
...
function DELETE(data) {
	$.ajax({
		url : "/SpringRestHibernateExample/deleteTrainers/" + data.id,
		method : "DELETE",
		contentType : "application/json",
		success : function() {
...
$('#clear').click(function(e){
	e.preventDefault();
	$('#trainers-image').css('display','none');
});
